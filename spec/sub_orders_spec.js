...
frisby.create('Correct order creation')
    ...
    .afterJSON(function(order) {
      frisby.create('No team specified in suborder creation')
        .post(url + '/orders/' + order.id + '/suborder', {'allocatable_budget': 50, 'name' : "new order"})
        .expectStatus(422)
      ...
      frisby.create('No allocatable_budget specified in suborder creation')
        .post(url + '/orders/' + order.id + '/suborder', {'team': {'id' : 2}, 'name' : "new order"})
        .expectStatus(422)
        .expectJSON({error:'ORDER_ERROR'})
      ...
      frisby.create('No order name specified in suborder creation')
        .post(url + '/orders/' + order.id + '/suborder', {'allocatable_budget': 50, 'team': {'id' : 2}})
        .expectStatus(422)
        .expectJSON({error:'ORDER_ERROR'})
      ...
      frisby.create('Create correct suborder')
        ...
        .afterJSON(function(subOrder) {
          ...
          frisby.create('Suborder can not be created from suborder')
            .post(url + '/orders/' + subOrder.id + '/suborder', {'allocatable_budget': 50, 'team' : {'id' : 3},  'name' : 'super order'})
            .expectStatus(422)
            .expectJSON({error:'ORDER_ERROR'})
          ...
          frisby.create('Suborder can not be invoiced more than parent free budget')
            .post(url + '/orders/' + order.id + '/suborder', {'allocatable_budget': 500, 'team' : {'id' : 3}, 'name' : 'super order'})
            .expectStatus(422)
            .expectJSON({error:'ORDER_ERROR'})
          ...
          frisby.create('Create second suborder from parent order')
            .post(url + '/orders/' + order.id + '/suborder', {'allocatable_budget': 20, 'team' : {'id' : 2}, 'name' : 'super order'})
            .expectStatus(201)
            .afterJSON(function(subOrder) {
              frisby.create('Allocatable budget on parent order should decrease')
                .get(url + '/orders/' + order.id)
            ...
            .patch(url + '/orders/' + subOrder.id, {'allocatable_budget': 22, 'invoiced_budget' : 22})
            .expectStatus(200)
            .afterJSON(function(){
              frisby.create('Allocatable budget on parent order should decrease')
                .get(url + '/orders/' + order.id)
          ...
          frisby.create('Update suborder budget to more than available')
            .patch(url + '/orders/' + subOrder.id, {'allocatable_budget': 500 , 'invoiced_budget' : 500})
            .expectStatus(422)
            .expectJSON({error:'ORDER_ERROR'})
            .expectBodyContains('Suborder can not be invoiced more than parent free budget')
            .toss();
        ...
        frisby.create('Suborder can not be created for the same team as parent order')
            .post(url + '/orders/' + order.id + '/suborder', {'allocatable_budget': 50, 'team' : {'id' : 1},  'name' : 'super order'})
            .expectStatus(422)
            .expectJSON({error:'ORDER_ERROR'})
