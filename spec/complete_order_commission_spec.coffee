    ...
    frisby.create('Correct order creation')
      ...
      .afterJSON (order)->
         frisby.create('Invoice order with correct invoice')
           ...
           .expectStatus(200)
           .toss()
         frisby.create('Set order commission 10%')
           .post(url + '/order/' + order.id + '/commission', {'commission' : 10})
           .expectStatus(200)
           .toss()
