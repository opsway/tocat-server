...
module TimeLog
  class ZohoLeaveApi
    ...
    def create_paid_leave
      ...
      data = "&xmlData=<Request><Record><field name='Employee_ID'>#{employee_id}</field><field name='To'>#{@params['date']}</field><field name='From'>#{@params['date']}</field><field name='Leavetype'>#{leave_type_id}</field><days><date name='#{@params['date']}'>#{@params['percentage']}</date></days></Record></Request>"
    ...
    def approve_created_leave(leave_id)
      ...
      request = RestClient.post(url, {timeout: 10})
      p request
      request
    end
